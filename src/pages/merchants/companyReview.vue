<route>
  ...
  const GetList = async () => {
    try {
      loading.value = true
      const response: any = await useAxios().get('/default/shop-edit/company-info', { params: Query.value })
      loading.value = false
  ...
  const review = async () => {
    ...
    try {
      ...
      const response: any = await useAxios().put('/default/shop-edit/audit-company-info', submit)
  ...
  <template>
    ...
    <div class="mt-4 min-h-[320px]" v-loading="loading">
      <el-table :data="items" stripe>
        ...
        <el-table-column label="身份证正面照" align="center" width="110">
          <template #default="{ row }">
            <el-image
        ...
        <el-table-column label="身份证反面照" align="center" width="110">
          <template #default="{ row }">
            <el-image
