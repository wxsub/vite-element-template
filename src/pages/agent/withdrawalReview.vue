<route>
  ...
  const GetList = async () => {
    try {
      loading.value = true
      const response: any = await useAxios().get('/wallet/manager-salary/withdrawals', { params: Query.value })
      loading.value = false
  ...
  const review = async () => {
    ...
    try {
      ...
      const response: any = await useAxios().put(`/wallet/manager-salary/withdrawal-audit/${id}`, reviewDailog.value)
