<route>
  ...
  const GetList = async () => {
    try {
      loading.value = true
      const response: any = await useAxios().get('/wallet/mall/subsidy/withdrawals', { params: Query.value })
      loading.value = false
  ...
  const review = async () => {
    ...
    try {
      ...
      const response: any = await useAxios().put(`/wallet/mall/subsidy/withdrawal-audit/${id}`, reviewDailog.value)
  ...
  <template>
    ...
    <div class="mt-4 min-h-[320px]" v-loading="loading">
      <el-table :data="items" stripe>
        ...
        <el-table-column prop="bankName" label="提现银行" align="center" />
        <el-table-column prop="openUserName" label="开户人姓名" align="center" width="120" />
        <el-table-column prop="bankCardNo" label="银行卡号" align="center" />
        <el-table-column prop="bankSubName" label="提现银行支行" align="center" width="200" />
        <el-table-column label="开户行地址" align="center" show-overflow-tooltip width="120">
            <template #default="{ row }">
                <span>{{ row.openProvince + row.openCity }}</span>
            </template>
        </el-table-column>
