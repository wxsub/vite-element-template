...
const GetList = async () => {
    try {
        loading.value = true
        const response: any = await useAxios().get(`/default/local-trend/single-achievements`, {
...
const fetchAchievements = async () => {
    try {
        ...
        const id = dialogDataSet.id || ''
        const response: any = await useAxios().get(`/default/local-trend/single/${id}/achievements-detail`, {
...
<template>
    ...
    <el-dialog v-model="dialogDataSet.visible" title="个人业绩明细" :close-on-click-modal="false" width="520">
        <div class="min-h-[320px]" v-loading="dialogDataSet.loading">
            <el-table :data="dialogDataSet.items" stripe>
                <el-table-column prop="agentName" label="商务名称" align="center" />
                <el-table-column prop="orderNo" label="订单编号" align="center" />
                <el-table-column prop="points" label="业绩" align="center" />
